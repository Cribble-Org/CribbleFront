...
export default function CommunitiesDashboard() {
  ...
  const hideDashboardModal = JSON.parse(
    localStorage.getItem("hideDashboardModal") ?? "false"
  );
  ...
  useEffect(() => {
    if (getAccessToken()) setOpenCommunitiesModal(!hideDashboardModal);
  }, [hideDashboardModal]);
  ...
  const toggleFavorite = (id: string) => {
    dispatch(handleFavoriteChannel({ selectedChannelId: id })).then(
      (response) => {
  ...
  const toggleSelect = (
    ...
    const selected = selectedCommunities.includes(id)
      ? selectedCommunities.filter((cId) => cId !== id)
  ...
  const refreshList = async (userId: string) => {
    const response = await dispatch(getRefreshUserData({ userId }));
    if (response?.payload?.data?.allChannels) {
      const sortedChannels = [...response.payload.data.allChannels].sort(
        (a, b) => {
  ...
  const getSentimentsActiveness = (ids?: string[]) => {
    ...
    const formattedStartDate = startDate
      ? `${adjustToLocalDate(new Date(startDate))}`
      : undefined;
    const formattedEndDate = endDate
      ? `${adjustToLocalDate(new Date(endDate))}`
      : undefined;
  ...
  const updateChannels = (
    ...
    dispatch(saveChannelsAPI(updatedChannels))
      .then((response) => {
      ...
      .finally(() => dispatch(setChannelLoadingTable(false)));
  };
    ...
    <div className="w-full text-white p-6 rounded-xl">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {!hideChannelTable && (
          <div className="flex space-x-2">
            ...
            >
              <DropdownMenuItem onClick={() => getSentimentsActiveness()}>
                Add to Dashboard
              </DropdownMenuItem>
            ...
            <Button
              className="bg-[#252134] border-gray-700"
              onClick={() => refreshList(userData?.id || "")}
            >
              <RefreshCcw size={20} />
            </Button>
          </div>
        ...
        <>
                      ...
                      >
                        <td className="py-4">
                          <div className="flex items-center">
                            <Checkbox
                              ...
                              onChange={(event) =>
                                toggleSelect(event, community.id)
                              }
                            />
                        ...
                        <td className="py-4">
                          {community?.role?.map((r, index) => (
                            <span
                              key={index}
                              className="inline-block w-6 h-6 rounded-full bg-gray-700 text-center mr-1"
                            >
                              {r}
                            </span>
                          ))}
                        ...
                        <td className="py-4">
                          {community.participants.toLocaleString()}
                        </td>
                        <td className="py-4">
                          <Button
                            ...
                            className={getStatusStyles(
                              community?.isActive ? "Activated" : "Deactivated"
                            )}
                            onClick={() => {
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mt-4">
            <div className="text-sm text-gray-400">
              Showing {indexOfFirstCommunity + 1} to{" "}
              {Math.min(indexOfLastCommunity, communities.length)} of{" "}
      ...
      {openConfirmModal.open && (
        <ActiveChannelModal
          ...
          onConfirm={
            openConfirmModal.active ? activateChannels : deactivateChannel
          }
