...
import {
  ...
  GET_CHANNELS_URL,
  GET_DB_CHANNELS_URL,
  GET_REFRESH_USER_DATA,
  GET_SAVE_CHANNELS_URL,
  SAVE_CHANNELS_URL,
  SENTIMENTS_URL,
} from "../../constants/apis";
...
>("getChannels", async (_params, { rejectWithValue }) => {
  try {
...
export const getDBChannels = createAsyncThunk<
  ...
  void,
  { rejectValue: ApiError }
>("getDBChannels", async (_params, { rejectWithValue }) => {
  try {
...
export const saveChannelsAPI = createAsyncThunk(
  ...
  async (params: string[], { rejectWithValue }) => {
    try {
      const { data } = await Axios.post(SAVE_CHANNELS_URL, {
        selectedChannelIds: params,
      });
      return data;
    } catch (error) {
      if (error instanceof AxiosError && error.response) {
...
export const getRefreshUserData = createAsyncThunk(
  "getRefreshUserDataAPI",
  async (params: { userId: string }, { rejectWithValue }) => {
    try {
      if (!params?.userId) {
        return rejectWithValue({ message: "User ID is required" } as ApiError);
      }
      const { data } = await Axios.get(GET_REFRESH_USER_DATA, {
